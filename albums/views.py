...
class AlbumAdminMixin(LoginRequiredMixin, UserPassesTestMixin):
    """Only the album owner or a collaborator with admin role can pass."""
    def test_func(self):
...
class AlbumContributorMixin(LoginRequiredMixin, UserPassesTestMixin):
    """Owner, admin collaborators, and contributors can pass."""
    def test_func(self):
        album = get_object_or_404(Album, pk=self.kwargs['pk'])
        if album.owner == self.request.user:
            return True
...
class DashboardView(LoginRequiredMixin, TemplateView):
    template_name = 'albums/dashboard.html'
...
class AlbumDeleteView(AlbumAdminMixin, DeleteView):
    ...
    def form_valid(self, form):
        ...
        # Delete all Cloudinary images before deleting the album
...
class PhotoUploadView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def form_valid(self, form):
        album   = get_object_or_404(Album, pk=self.kwargs['pk'])
        files   = self.request.FILES.getlist('image')  # get ALL files
...
class PhotoEditView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        if album.owner == self.request.user:
            return True
        return album.album_collaborators.filter(user=self.request.user, role__in=['admin', 'contributor']).exists()
...
class SetCoverView(LoginRequiredMixin, UserPassesTestMixin, View):
    def test_func(self):
        photo = get_object_or_404(Photo, pk=self.kwargs['pk'])
        album = photo.album
        if album.owner == self.request.user:
